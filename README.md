...
- Release: `1.0.3` (manifest, package, tag, and `main` agree).
- Minimum Obsidian version: `1.11.4`.
- Mobile support: enabled (`isDesktopOnly: false`).
- Automated evidence: 42 Vitest test files and 467 passing tests in the current tree.
...
The source computes the canonical storage path from `app.vault.configDir`. One current limitation remains documented in the source of truth: the default exclusion constant is initialized from a `.obsidian` fallback, so custom configuration-directory exclusion needs a production follow-up before it can be described as fully covered.
...
PAT redaction is implemented by `src/security/redact.ts` and used by GitHub/UI error paths. The current source still has diagnostic warning sites that pass error objects directly; therefore the project does not claim that every console diagnostic is blanket-redacted until that follow-up is completed.
