...
[![Version](https://img.shields.io/badge/version-1.0.7-blue.svg)](https://github.com/ankhang0704/github-vault-relay/releases)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

## Current canonical baseline

- Release: `1.0.7` (manifest, package, tag, and `main` agree after release).
- Minimum Obsidian version: `1.11.4`.
- Mobile support: enabled (`isDesktopOnly: false`).
- Automated evidence: 45 Vitest test files and 495 passing tests in the current tree.
...
- Release-gate physical acceptance is PASS on Windows and iPhone for settings, hidden paths, normal files, and restart behavior. See [the manual matrix](docs/MANUAL_TEST_MATRIX.md) for the recorded sign-off.
...
- **Optional Desktop Git Integration**: For vaults that also contain a `.git` repository on Desktop, an optional background task can automatically advance local Git metadata (`git fetch` + `git reset --mixed <remoteCommitSha>`) after a successful sync, eliminating stale Git status debt without re-downloading files. This is strictly opt-in, non-blocking, and 100% isolated/hidden on mobile.
