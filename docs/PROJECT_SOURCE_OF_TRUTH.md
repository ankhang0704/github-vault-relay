...
| Release | `1.0.8` | `manifest.json`, `package.json`, Git tag `1.0.8` after release |
...
| Test files | `45` | `tests/*.test.ts`, current `vitest run` output |
| Passing tests | `496` | current `vitest run` output |
| Quality gate | `PASS` (`npm run verify`, 2026-09-07) | `package.json`, local gate run |

The stable 1.0.8 release is published at `https://github.com/ankhang0704/github-vault-relay/releases/tag/1.0.8`. Provenance is generated by `.github/workflows/release.yml` from immutable tag `1.0.8`.

`main.js` is a generated, ignored build artifact. The current production build identity is recorded by the verification run and should be regenerated rather than hand-edited. The manual matrix records release asset testing for reproducible device verification.
...
It runs ESLint with zero warnings, TypeScript typechecking, Vitest, and the production build. Current automated test evidence is 44 files and 480 passing tests. Historical checkpoint totals in `docs/development-history/` and older Changelog entries are snapshots, not current totals.

The current manual acceptance matrix records the release-gate results for Windows and iPhone. Any matrix row not explicitly included in a physical sign-off remains `NOT RUN`; automated tests do not replace physical acceptance.
...
The 1.0.6 release resolves directory move desynchronization and duplicate note creation by implementing baseline auto-healing across converged files in `UnifiedSyncEngine` and `PushEngine`, NFC Unicode normalization in path filtering, and Windows case-insensitive baseline lookup. All prior 1.0.5 regression findings remain covered by focused tests.
