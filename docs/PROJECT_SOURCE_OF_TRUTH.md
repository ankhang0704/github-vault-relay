...
| Release | `1.0.7` | `manifest.json`, `package.json`, Git tag `1.0.7` after release |
...
| Passing tests | `495` | current `vitest run` output |
| Quality gate | `PASS` (`npm run verify`, 2026-09-07) | `package.json`, local gate run |

The stable 1.0.7 release is published at `https://github.com/ankhang0704/github-vault-relay/releases/tag/1.0.7`. Provenance is generated by `.github/workflows/release.yml` from immutable tag `1.0.7`.
